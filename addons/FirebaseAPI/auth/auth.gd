		...
		if not res.has("kind"):
			...
			match res.kind:
				...
				RESPONSE_USERDATA:
					...
					Global.logged_in_user = userdata
					Global.logged_in.emit()
			auth_request.emit(1, auth)
	else:
...
func save_auth(auth: Dictionary) -> void:
	...
	var err = FileAccess.open_encrypted_with_pass("user://user.auth", FileAccess.WRITE, Global.firebaseConfig.apiKey)
	#print(err.get_as_text())
	...
	if err == null:
		...
		#print(encrypted_file.get_as_text())
		encrypted_file.close()
...
func load_auth() -> void:
	var encrypted_file = FileAccess.open("user://user.auth", FileAccess.READ)
	var err = encrypted_file.open_encrypted_with_pass("user://user.auth", FileAccess.READ, Global.firebaseConfig.apiKey)
	if err == null:
		Firebase._printerr("Error Opening Firebase Auth File. Error Code: " + str(err))
		emit_signal("auth_request", err, "Error Opening Firebase Auth File.")
	else:
		
		#print(encrypted_file.get_line())
		#print(JSON.parse_string("GDEC‘åŸè"))
		var line = get_line(encrypted_file)
		var encrypted_file_data = JSON.parse_string(line)
		manual_token_refresh(encrypted_file_data)

func get_line(encrypted_file):
	#var encrypted_file = FileAccess.open("user://user.auth", FileAccess.READ)
	while encrypted_file.get_position() < encrypted_file.get_length():
			var line = encrypted_file.get_line().strip_edges() # Read and clean up the line
			if 'n":' in line:
				var index = line.find('n"')# + len('n":"3600",')
				return '{"' + line.substr(index) # Return the substring starting with '{"n":"3600'
	
	#print(encrypted_file.get_position())
	#print(encrypted_file.get_length())
	#print(encrypted_file.get_position() < encrypted_file.get_length())
...
func remove_auth() -> void:
	...
	if dir and dir.file_exists("user.auth"):
		dir.remove("user://user.auth")
	else:
		Firebase._printerr("No encrypted auth file exists")
...
func check_auth_file() -> bool:
	var dir = DirAccess.open("user://")
	if dir and dir.file_exists("user.auth"):
		# Will ensure "auth_request" is emitted
		load_auth()
		return true
	else:
		Firebase._printerr("Encrypted Firebase Auth file does not exist")
